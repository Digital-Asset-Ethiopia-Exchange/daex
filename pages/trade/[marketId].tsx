...
const Trade = () => {
  ...
  const id = router.query.marketId;
  const baseCurrency = (id as string)?.split("_")[0];
  const quoteCurrency = (id as string)?.split("_")[1];
  ...
  return (
    ...
    >
      <div key="order-form" className="bg-gray-100">
        <OrderForm baseCurrency={baseCurrency} quoteCurrency={quoteCurrency} />
      </div>
      ...
      <div
        key="trades"
        className="border bg-gray-50 flex flex-col h-full font-mono font-bold sm:font-semibold text-sm px-4 py-5 overflow-hidden"
      >
        <Trades baseCurrency={baseCurrency} quoteCurrency={quoteCurrency} />
      </div>
